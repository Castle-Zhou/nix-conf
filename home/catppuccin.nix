# 全局 catppuccin 风味/强调色 + 官方四套色板（单一数据源）
# 由 theme.nix（GTK/Qt/图标/dconf）和 niri.nix（waybar style.css 模板）共同 import。
# 换主题只改这里的 flavor；改完 rebuild，GTK/Qt/waybar 全部跟随。

{
  flavor = "latte";   # "latte" | "frappe" | "macchiato" | "mocha"
  accent = "mauve";

  # 官方 palette（catppuccin/palette），hex 值
  palettes = {
    latte = {
